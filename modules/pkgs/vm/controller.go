...
func (c *VmController) object(vm string) dbus.BusObject {
	...
	return c.conn.Object("ae.tii.MemManager", vmObj)
}
...
func (c *VmController) VMSize(ctx context.Context, vm string, minimum *uint64, maximum *uint64) (*givc_vm.VMSizeResponse, error) {
	...
	if minimum != nil {
		if err := vmObj.SetProperty("ae.tii.MemManager.VM.Minimum", *minimum); err != nil {
			return nil, err
		}
		ret.Minimum = *minimum
	} else {
		val, err := vmObj.GetProperty("ae.tii.MemManager.VM.Minimum")
		if err != nil {
			return nil, err
		}
	...
	if maximum != nil {
		if err := vmObj.SetProperty("ae.tii.MemManager.VM.Maximum", *maximum); err != nil {
			return nil, err
		}
		ret.Maximum = *maximum
	} else {
		val, err := vmObj.GetProperty("ae.tii.MemManager.VM.Maxmium")
		if err != nil {
			return nil, err
		}
		uval, _ := val.Value().(uint64)
	...
	val, err := vmObj.GetProperty("ae.tii.MemManager.VM.MaxSize")
	if err != nil {
		return nil, err
	}
	uval, _ := val.Value().(uint64)
...
func (c *VmController) VMStats(ctx context.Context, vm string) (*map[string]uint64, error) {
	...
	err := vmObj.Call("ae.too.MemManager.VM.GetStats", 0).Store(&stats)
	if err != nil {
		return nil, err
	}
